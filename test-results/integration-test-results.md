...
Generated at: 2025-11-07T16:26:50.384Z
...
|------|--------|----------|
| should have required methods for update management | ✅ passed | 0.01s |
...
| Test | Status | Duration |
|------|--------|----------|
| should set and get database path correctly | ✅ passed | 0.08s |
...
| should create database file if it does not exist | ✅ passed | 0.01s |

#### Database Module > Schema Management

| Test | Status | Duration |
|------|--------|----------|
| should create timesheet table with correct schema | ✅ passed | 0.04s |
| should create required indexes | ✅ passed | 0.03s |
| should create unique constraint for deduplication | ✅ passed | 0.04s |

#### Database Module > Timesheet Entry Insertion

| Test | Status | Duration |
|------|--------|----------|
| should insert a new timesheet entry successfully | ✅ passed | 0.05s |
| should calculate hours automatically | ✅ passed | 0.03s |
| should handle optional fields correctly | ✅ passed | 0.04s |
| should validate time constraints | ✅ passed | 0.05s |

#### Database Module > Deduplication Functionality

| Test | Status | Duration |
|------|--------|----------|
| should prevent duplicate entries based on unique constraint | ✅ passed | 0.22s |
| should allow entries with different time_in | ✅ passed | 0.15s |
| should allow entries with different project | ✅ passed | 0.06s |
| should allow entries with different task description | ✅ passed | 0.05s |
| should allow entries with different date | ✅ passed | 0.03s |
| should allow entries with different optional fields | ✅ passed | 0.05s |

#### Database Module > Duplicate Checking Utilities

| Test | Status | Duration |
|------|--------|----------|
| should correctly identify non-duplicate entries | ✅ passed | 0.02s |
| should correctly identify duplicate entries | ✅ passed | 0.03s |
| should find duplicate entries in database | ✅ passed | 0.04s |
...
| Test | Status | Duration |
|------|--------|----------|
| should insert multiple entries successfully | ✅ passed | 0.06s |
| should handle mixed duplicates in batch insertion | ✅ passed | 0.06s |
| should handle empty batch | ✅ passed | 0.06s |
| should use transaction for atomicity | ✅ passed | 0.13s |

#### Database Module > Edge Cases and Error Handling

| Test | Status | Duration |
|------|--------|----------|
| should handle database connection errors gracefully | ✅ passed | 0.11s |
| should handle malformed entry data | ✅ passed | 0.04s |
| should handle null and undefined values in optional fields | ✅ passed | 0.03s |

#### Database Module > Concurrent Access Tests

| Test | Status | Duration |
|------|--------|----------|
| should handle concurrent reads safely | ✅ passed | 0.03s |
| should handle concurrent writes with proper isolation | ✅ passed | 0.12s |
| should maintain data consistency under concurrent operations | ✅ passed | 0.06s |

#### Database Module > Transaction Rollback Tests

| Test | Status | Duration |
|------|--------|----------|
| should rollback transaction on batch insertion failure | ✅ passed | 0.06s |
| should commit all or none in batch operation | ✅ passed | 0.06s |

#### Database Module > Database Corruption Recovery Tests

| Test | Status | Duration |
|------|--------|----------|
| should detect corrupted database schema | ✅ passed | 0.04s |
| should handle missing tables gracefully | ✅ passed | 0.04s |
| should validate schema integrity on startup | ✅ passed | 0.02s |
| should handle constraint violations appropriately | ✅ passed | 0.04s |

#### Database Module > Migration Tests

| Test | Status | Duration |
|------|--------|----------|
| should maintain schema version compatibility | ✅ passed | 0.02s |
| should preserve existing data after schema changes | ✅ passed | 0.02s |
| should handle schema recreation without data loss | ✅ passed | 0.08s |
...
| Test | Status | Duration |
|------|--------|----------|
| should have BASE_URL set to deprecated placeholder | ✅ passed | 0.01s |
...
### C:/Users/andrew.hughes/Program Development/Sheetpilot/app/backend/tests/import-policy.spec.ts

#### Import policy - late import of logger and runtime deps

| Test | Status | Duration |
|------|--------|----------|
| main.ts must not statically import ../../shared/logger at top-level | ✅ passed | 0.01s |
| preflight should exist to resolve critical modules | ✅ passed | 0.00s |
...
| should handle duplicate entries | ✅ passed | 0.01s |
...
| Test | Status | Duration |
|------|--------|----------|
| timesheet:submit returns error if credentials missing | ✅ passed | 0.01s |
| timesheet:submit submits with stored credentials | ✅ passed | 0.01s |
...
| Test | Status | Duration |
|------|--------|----------|
| should handle workflow when database has pending entries | ✅ passed | 0.15s |
| should handle workflow when database is empty | ✅ passed | 0.33s |
| should not mutate database entries during failed submission | ✅ passed | 0.19s |
| should handle multiple pending entries with different projects | ✅ passed | 0.09s |
| should maintain data integrity across automation attempts | ✅ passed | 0.09s |

#### IPC Workflow Integration > Concurrent IPC Calls

| Test | Status | Duration |
|------|--------|----------|
| should handle concurrent read operations | ✅ passed | 0.17s |
| should handle concurrent write operations | ✅ passed | 0.12s |
| should maintain consistency under concurrent access | ✅ passed | 0.29s |

#### IPC Workflow Integration > IPC Call Ordering

| Test | Status | Duration |
|------|--------|----------|
| should process calls in correct order | ✅ passed | 0.12s |
| should maintain FIFO order for queued operations | ✅ passed | 0.03s |

#### IPC Workflow Integration > IPC Timeout Handling

| Test | Status | Duration |
|------|--------|----------|
| should handle long-running operations | ✅ passed | 0.15s |
| should timeout excessively long operations | ✅ passed | 0.04s |
| should clean up on timeout | ✅ passed | 0.15s |
...
| Test | Status | Duration |
|------|--------|----------|
| should parse valid time strings correctly | ✅ passed | 0.01s |
...
| should save window state successfully (async with debouncing) | ✅ passed | 0.52s |
| should handle save errors gracefully (async) | ✅ passed | 0.51s |
...
| Test | Status | Duration |
|------|--------|----------|
| should have Q1, Q2, Q3 and Q4 2025 defined | ✅ passed | 0.01s |
...
| Test | Status | Duration |
|------|--------|----------|
| should require formConfig parameter (no longer optional) | ✅ passed | 0.01s |
...
| should pass formConfig to WebformFiller | ✅ passed | 0.01s |
...
| Test | Status | Duration |
|------|--------|----------|
| should fetch pending entries correctly | ✅ passed | 0.07s |
| should mark entries as submitted correctly | ✅ passed | 0.05s |
| should remove failed entries correctly | ❌ failed | 0.11s |
...
| should initialize and cleanup browser when submitting timesheets | ✅ passed | 0.05s |
| should properly handle browser lifecycle across multiple submission attempts | ✅ passed | 0.13s |
...
| Test | Status | Duration |
|------|--------|----------|
| returns true when current URL includes any success pattern | ✅ passed | 0.02s |
| returns true (default) even if patterns not matched (back-compat) | ✅ passed | 0.01s |
| returns false when require_page throws | ✅ passed | 0.02s |
...
| Test | Status | Duration |
|------|--------|----------|
| validate required fields logic | ✅ passed | 0.01s |
| project-specific tool locator resolution | ✅ passed | 0.00s |
| should return error when run_automation is called without start() | ✅ passed | 0.03s |
| should work when start() is called before run_automation() | ✅ passed | 10.04s |
...
| Test | Status | Duration |
|------|--------|----------|
| should allow start() to be called multiple times safely | ✅ passed | 10.07s |
| should allow close() to be called multiple times safely | ✅ passed | 10.04s |
| should handle close() when start() was never called | ✅ passed | 0.01s |
| should prevent operations after close() | ✅ passed | 10.03s |
| should properly cleanup resources on automation error | ✅ passed | 10.03s |
| should handle concurrent automation attempts gracefully | ✅ passed | 10.04s |
| should initialize chromium browser correctly | ✅ passed | 10.02s |
| should handle headless mode configuration correctly | ✅ passed | 20.04s |
...
| Test | Status | Duration |
|------|--------|----------|
| succeeds when first submit fails and second succeeds (one retry) | ✅ passed | 0.19s |
| fails when both first and retry submissions fail | ✅ passed | 0.09s |
...
| Test | Status | Duration |
|------|--------|----------|
| should handle empty rows array gracefully | ✅ passed | 0.03s |
| should return proper error structure when authentication fails | ✅ passed | 10.06s |
| should handle invalid credentials gracefully | ✅ passed | 10.04s |
| should process multiple rows and report individual failures | ✅ passed | 10.03s |
...
```
expected [ { date: '2025-01-15', …(10) } ] to have a length of +0 but got 1
```
