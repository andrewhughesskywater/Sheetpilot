 ...
 */
export const PROJECTS_WITHOUT_TOOLS = ['ERT', 'PTO/RTO', 'SWFL-CHEM/GAS', 'Training'] as const;
...
export function doesProjectNeedTools(project: string): boolean {
  return Boolean(project) && !isProjectWithoutTools(project);
}
...
export function doesToolNeedChargeCode(tool: string): boolean {
  return Boolean(tool) && !isToolWithoutChargeCode(tool);
}
