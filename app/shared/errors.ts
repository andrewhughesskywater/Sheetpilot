...
export abstract class AppError extends Error {
  ...
  constructor(message: string, code: string, category: ErrorCategory, context: Record<string, unknown> = {}) {
    super(message);
...
export class CredentialsRetrievalError extends CredentialsError {
  constructor(service: string, context: Record<string, unknown> = {}) {
    super(`Could not retrieve credentials for service: ${service}`, 'CRED_RETRIEVAL_ERROR', { ...context, service });
  }
}
...
export class InvalidCredentialsError extends CredentialsError {
  constructor(context: Record<string, unknown> = {}) {
    super('Invalid credentials provided', 'CRED_INVALID', context);
  }
}
...
export class SubmissionServiceUnavailableError extends SubmissionError {
  constructor(service: string, context: Record<string, unknown> = {}) {
    super(`Submission service unavailable: ${service}`, 'SUBMISSION_SERVICE_UNAVAILABLE', { ...context, service });
  }
}
...
export class SubmissionFailedError extends SubmissionError {
  constructor(reason: string, context: Record<string, unknown> = {}) {
    super(`Could not submit timesheet entries: ${reason}`, 'SUBMISSION_FAILED', { ...context, reason });
  }
}
...
export class NoEntriesToSubmitError extends SubmissionError {
  constructor(context: Record<string, unknown> = {}) {
    super('No pending timesheet entries to submit', 'SUBMISSION_NO_ENTRIES', context);
  }
}
...
export class InvalidDateError extends ValidationError {
  constructor(date: string, context: Record<string, unknown> = {}) {
    super(`Invalid date format: ${date}. Expected MM/DD/YYYY`, 'VALIDATION_INVALID_DATE', { ...context, date });
  }
}
...
export class InvalidTimeError extends ValidationError {
  constructor(time: string, context: Record<string, unknown> = {}) {
    super(`Invalid time format: ${time}. Expected HH:MM or HHMM in 15-minute increments`, 'VALIDATION_INVALID_TIME', {
      ...context,
      time,
    });
  }
...
export class RequiredFieldError extends ValidationError {
  constructor(field: string, context: Record<string, unknown> = {}) {
    super(`Required field missing: ${field}`, 'VALIDATION_REQUIRED_FIELD', { ...context, field });
  }
}
...
export class InvalidFieldValueError extends ValidationError {
  constructor(field: string, value: unknown, context: Record<string, unknown> = {}) {
    super(`Invalid value for field '${field}': ${String(value)}`, 'VALIDATION_INVALID_VALUE', {
      ...context,
      field,
      value,
    });
...
export class IPCHandlerNotFoundError extends IPCError {
  constructor(handler: string, context: Record<string, unknown> = {}) {
    super(`IPC handler not found: ${handler}`, 'IPC_HANDLER_NOT_FOUND', { ...context, handler });
  }
}
...
export class IPCCommunicationError extends IPCError {
  constructor(endpoint: string, context: Record<string, unknown> = {}) {
    super(`IPC communication failed: ${endpoint}`, 'IPC_COMMUNICATION_ERROR', { ...context, endpoint });
  }
}
...
export function isRetryableError(error: unknown): boolean {
  if (error instanceof AppError) {
    return error.category === ErrorCategory.NETWORK || error.category === ErrorCategory.DATABASE;
  }
