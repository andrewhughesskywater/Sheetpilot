...
function extractFromSingleObject(obj: Record<string, unknown>): {
  message: string;
...
function extractFromMultipleArgs(data: unknown[]): { message: string; context?: Record<string, unknown> } {
  ...
  const context =
    data.length > 1 && typeof data[1] === 'object' && data[1] !== null
 ...
 * @private
 */
function extractMessageAndContext(data: unknown[]): {
  message: string;
  context?: Record<string, unknown>;
  component?: string;
} {
...
export function configureLogger() {
  ...
  log.errorHandler.startCatching({
    ...
    onError: (options: {
      ...
      createIssue: (url: string, data: unknown) => void;
    }) => {
