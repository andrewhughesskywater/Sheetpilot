...
export async function processEntriesByQuarter(
  ...
  botLogger.verbose("Entries grouped by quarter", {
  ...
  for (const [quarterId, quarterEntries] of Array.from(
    quarterGroups.entries()
  )) {
    ...
    const quarterDef = quarterEntries[0]
      ? getQuarterForDate(quarterEntries[0].date)
      : null;
    if (!quarterDef) {
    ...
    if (config.useMockWebsite) {
      ...
      const mockFormId =
        process.env["MOCK_FORM_ID"] || "0197cbae7daf72bdb96b3395b500d414";
      botLogger.info("Using mock website for submission", {
        mockBaseUrl,
        mockFormId,
      });
    ...
    try {
      ...
      botLogger.info("Submission aborted during quarter processing", {
        quarterId,
      });
