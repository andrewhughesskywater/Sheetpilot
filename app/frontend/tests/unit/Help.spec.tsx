...
describe('Help Component', () => {
  ...
  describe('Log Export Functionality', () => {
    ...
    it('should handle log path API unavailable', async () => {
      ...
      const hasLogsAPI =
        (global as { window?: { logs?: { getLogPath?: unknown } } }).window?.logs?.getLogPath !== undefined;
    ...
    it('should construct full log path correctly', () => {
      ...
      const fullPath = logPath.endsWith('\\') ? logPath + logFile : logPath + '\\' + logFile;
  ...
  describe('Credential Update Functionality', () => {
    ...
    it('should prefill email from existing credentials', async () => {
      mockWindow.credentials.list.mockResolvedValue({
        success: true,
        credentials: [{ id: 1, service: 'smartsheet', email: 'existing@test.com', created_at: '', updated_at: '' }],
      });

      const response = await mockWindow.credentials.list();
      const existingCred = response.credentials.find(
        (c: { service: string; email?: string }) => c.service === 'smartsheet'
      );
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle service not found in credentials', () => {
      const credentials = [{ service: 'other', email: 'other@test.com' }];
