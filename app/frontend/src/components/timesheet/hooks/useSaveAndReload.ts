...
export function useSaveAndReload(
  ...
  const applySavedEntry = useCallback(
    (currentData: TimesheetRow[], currentRowIdx: number, savedEntry: TimesheetRow) => {
  ...
  const handleSaveSuccess = useCallback(
    (savedEntry: TimesheetRow, originalRow: TimesheetRow, rowIdx: number) => {
  ...
  const cleanupSaveOperation = useCallback(
    (rowIdx: number) => {
  ...
  const saveAndReloadRow = useCallback(
    async (row: TimesheetRow, rowIdx: number) => {
