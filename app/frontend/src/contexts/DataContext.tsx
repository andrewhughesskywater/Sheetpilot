...
export function DataProvider({ children }: DataProviderProps) {
  ...
  const handleDraftLoadSuccess = useCallback(
    (response: { success: boolean; entries?: TimesheetRow[]; error?: string }) => {
  ...
  const handleDraftLoadError = useCallback(
    (error: unknown) => {
  ...
  const parseArchiveResponse = useCallback(
    (archiveResponse: unknown): { timesheet: TimesheetEntry[]; credentials: Credential[] } => {
      const response = archiveResponse as {
        success?: boolean;
        timesheet?: TimesheetEntry[];
        credentials?: Credential[];
      };
  ...
  useInitializeDataContext(setTimesheetDraftData, setArchiveData, setIsTimesheetDraftLoading, setIsArchiveDataLoading);
