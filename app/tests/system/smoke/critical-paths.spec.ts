...
describe('Critical Path Smoke Tests', () => {
  ...
  // Helper to generate a random date string within the current quarter (MM/DD/YYYY)
  const randomDateInCurrentQuarter = (): string => {
    const now = new Date();
    ...
    const day = Math.floor(Math.random() * (end.getDate() - start.getDate() + 1)) + start.getDate();
  ...
  // Reuse one value across this file to avoid flakiness in perf assertions
