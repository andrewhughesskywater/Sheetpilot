...
export function createMainWindow(params: {
  ...
  const iconPath = params.packagedLike
    ? undefined
    : path.join(params.backendDirname, '..', '..', '..', '..', 'app', 'frontend', 'src', 'assets', 'images', 'icon.ico');
  ...
  if (!fs.existsSync(preloadPath)) {
    ...
    params.logger.error('Preload script not found', { preloadPath, backendDirname: params.backendDirname });
  ...
  window.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    ...
    if (isMainFrame) {
      try {
        ...
        params.logger.error('Could not show error dialog', { error: err instanceof Error ? err.message : String(err) });
      }
    }
  });
