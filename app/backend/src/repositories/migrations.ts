...
export function createBackup(db: BetterSqlite3.Database, dbPath: string): string | null {
  ...
  try {
    ...
    // Check if the database file exists (required in production and tests)
    if (!fs.existsSync(dbPath)) {
      dbLogger.warn('Database file does not exist; skipping backup', { dbPath });
      timer.done({ outcome: 'skipped', reason: 'missing-source' });
      return null;
    }

    // Additional validation in production (in tests, mocked fs doesn't track isFile properly)
    if (!isTestEnv) {
    ...
    // In test environments with mocked SQLite, skip the backup creation ops
    if (!isTestEnv) {
    ...
    // Verify backup was created (skip verification in test environments where fs is mocked)
    if (!isTestEnv && !fs.existsSync(backupPath)) {
