...
import { getDb , validateSession } from '../repositories';
...
export function registerDatabaseHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (event, token: string, options?: { page?: number; pageSize?: number }) => {
      if (!isTrustedIpcSender(event)) {
        return { success: false, error: 'Could not access database: unauthorized request', entries: [], totalCount: 0 };
      }
      // Validate session
      if (!token) {
        ipcLogger.security('database-access-denied', 'Unauthorized database access attempted', {
  ...
  ipcMain.handle('database:getAllArchiveData', async (event, token: string) => {
    ...
    try {
      ...
      const getTimesheet = db.prepare(
        "SELECT *, (time_out - time_in) / 60.0 as hours FROM timesheet WHERE status = 'Complete' ORDER BY date ASC, time_in ASC"
      );
