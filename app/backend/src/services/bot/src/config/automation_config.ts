...
export const FIELD_VALIDATION_FAIL_FAST: boolean = ['1', 'true', 'yes'].includes(
  (process.env['FIELD_VALIDATION_FAIL_FAST'] ?? 'true').toLowerCase()
);
...
export const AUTOMATION_STOP_ON_ROW_FAILURE: boolean = ['1', 'true', 'yes'].includes(
...
export const SCREENSHOT_DIRECTORY: string =
  process.env['SCREENSHOT_DIR'] ?? '\\\\swfl-file01\\\\Maintenance\\\\Python Programs\\\\logs\\\\screenshots';
...
export const LOGIN_STEPS: LoginStep[] = [
  ...
  { name: 'Stay Signed In — No', action: 'click', locator: '#idBtn_Back', expects_navigation: true, optional: true },
  {
    ...
    action: 'wait',
    element_selector: "input[aria-label='Project']",
    wait_condition: 'visible',
    optional: false,
  },
];
...
export const FIELD_ORDER: string[] = ['project_code', 'date', 'hours', 'tool', 'task_description', 'detail_code'];
...
export async function dynamic_wait(
  ...
  if (
    typeof condition_funcOrConfig === 'object' &&
    condition_funcOrConfig !== null &&
    'condition_func' in condition_funcOrConfig
  ) {
...
export async function wait_for_dom_stability(
  ...
  return dynamic_wait(
    async () => {
      try {
        ...
        if (initialRect) {
          ...
          if (finalRect && Math.abs(initialRect.x - finalRect.x) < 1 && Math.abs(initialRect.y - finalRect.y) < 1) {
            return true;
          }
        }
    ...
    Math.min(actualBaseTimeout, 1.0),
    Math.min(actualMaxTimeout, 2.0),
    DYNAMIC_WAIT_MULTIPLIER
  );
}
...
async function checkDOMActivity(page: import('playwright').Page): Promise<boolean> {
  try {
    ...
    return elements.some((el) => el.classList.length > 0 || el.getAttribute('style') || el.getAttribute('data-state'));
  } catch {
    return false;
  }
}
