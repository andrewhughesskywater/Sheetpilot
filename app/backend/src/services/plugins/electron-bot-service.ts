...
export class ElectronBotService implements ISubmissionService {
  public readonly metadata: PluginMetadata = {
    ...
    author: "Andrew Hughes",
    description:
      "Electron BrowserWindow-based browser automation submission service",
  };
  ...
  public async submit(
    ...
    botLogger.info("Starting Electron submission", {
  ...
  public validateEntry(entry: TimesheetEntry): ValidationResult {
    ...
    if (entry.timeIn && entry.timeOut) {
      ...
      const timeInMinutes =
        parseInt(timeInParts[0] || "0", 10) * 60 +
        parseInt(timeInParts[1] || "0", 10);
      const timeOutMinutes =
        parseInt(timeOutParts[0] || "0", 10) * 60 +
        parseInt(timeOutParts[1] || "0", 10);
