...
import type { ISubmissionService,SubmissionResult } from '@sheetpilot/shared/contracts/ISubmissionService';
import { formatMinutesToTime, normalizeDateToISO } from '@sheetpilot/shared/utils/format-conversions';
...
function validateSubmissionService(): ISubmissionService | null {
  ...
  botLogger.verbose('Retrieved submission service', {
    hasService: Boolean(submissionService),
    serviceName: submissionService?.metadata?.name,
    hasSubmit: Boolean(submissionService?.submit),
  });
...
async function submitViaPluginSystem(params: {
  ...
  const { submissionService, entries, credentials, progressCallback, abortSignal, useMockWebsite, dbRowCount, timer } =
    params;
