...
const packagedLike = process.resourcesPath !== undefined && 
  (__filename.includes('.asar') || process.resourcesPath.includes('.asar'));
...
if (packagedLike && !isSmoke) {
  ...
  modulePaths.push(
    pathModule.resolve(backendDirname, '..', '..', '..', '..', 'app', 'backend', 'node_modules')
  );
}
...
Module._resolveFilename = function(
  ...
  try {
    ...
    if (request.startsWith('@/')) {
      ...
      console.warn(`[Module Resolution] Compiled file not found for ${request}, expected at ${targetPath} or ${indexPath}`);
    }
    throw err;
  }
