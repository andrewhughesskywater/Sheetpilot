...
describe("Database Schema Contract Validation", () => {
  ...
  describe("Timesheet Table Schema", () => {
    ...
    it("should enforce correct data types", () => {
      ...
      expect(dbEntry.tool === null || typeof dbEntry.tool === "string").toBe(
        true
      );
      expect(
        dbEntry.detail_charge_code === null ||
          typeof dbEntry.detail_charge_code === "string"
      ).toBe(true);
      expect(
        dbEntry.status === null || typeof dbEntry.status === "string"
      ).toBe(true);
      expect(
        dbEntry.submitted_at === null ||
          typeof dbEntry.submitted_at === "string"
      ).toBe(true);
    });
    ...
    it("should store hours correctly", () => {
      const testCases = [
        { hours: 8.0, expected_hours: 8.0 },
        { hours: 0.25, expected_hours: 0.25 },
        { hours: 0.5, expected_hours: 0.5 },
        { hours: 1.0, expected_hours: 1.0 },
      ];

      testCases.forEach(({ hours, expected_hours }) => {
        expect(hours).toBe(expected_hours);
      });
    });
  });
  ...
  describe("Database Constraints", () => {
    ...
    it("should enforce unique constraint on natural key", () => {
      ...
      // Both entries have same natural key (date + project + task_description)
      const first = duplicateEntries[0]!;
      const second = duplicateEntries[1]!;
      const key1 = `${first.date}-${first.project}-${first.task_description}`;
      const key2 = `${second.date}-${second.project}-${second.task_description}`;
  ...
  describe("Index Performance", () => {
    ...
    it("should have unique index on natural key", () => {
      const uniqueIndexColumns = ["date", "project", "task_description"];
  ...
  describe("Data Integrity Validation", () => {
    ...
    it("should reject entries with constraint violations", () => {
      const invalidEntries = [
        DbTimesheetEntryBuilder.create().withHours(-1).build(), // Negative hours
        DbTimesheetEntryBuilder.create().withHours(25.0).build(), // Exceeds max 24.0
        DbTimesheetEntryBuilder.create().withHours(0.1).build(), // Not 15-minute increment
      ];
