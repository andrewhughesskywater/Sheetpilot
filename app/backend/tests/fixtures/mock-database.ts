      ...
      run: (params: unknown[]) => {
        if (sql.includes("INSERT INTO timesheet")) {
          const entry: DbTimesheetEntry = {
            ...
            hours: params[1] as number,
            project: params[2] as string,
            task_description: params[5] as string,
            ...((params[3] as string) ? { tool: params[3] as string } : {}),
            ...((params[4] as string)
              ? { detail_charge_code: params[4] as string }
              : {}),
            ...((params[6] as string) ? { status: params[6] as string } : {}),
            ...((params[7] as string)
              ? { submitted_at: params[7] as string }
              : {}),
          };
