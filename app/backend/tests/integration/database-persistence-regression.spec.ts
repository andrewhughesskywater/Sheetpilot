...
describe('Database Persistence Regression Tests', () => {
  ...
  describe('Regression: Entries disappearing after successful submission', () => {
    it('should reproduce the original bug scenario and verify fix', () => {
      ...
      const db = openDb();
      const archivedEntries = db.prepare('SELECT * FROM timesheet WHERE status = ?').all('Complete') as Array<{
      ...
      const archivedAfterReload = dbReload
        .prepare('SELECT * FROM timesheet WHERE status = ?')
        .all('Complete') as Array<{
        status?: string | null;
        submitted_at?: string | null;
        project?: string;
  ...
  describe('Regression: Data loss prevention', () => {
    ...
    it('should preserve all entry data during status transitions', () => {
      ...
      expect((submittedEntry as { detail_charge_code?: string | null }).detail_charge_code).toBe(
        originalEntry.detailChargeCode
      );
