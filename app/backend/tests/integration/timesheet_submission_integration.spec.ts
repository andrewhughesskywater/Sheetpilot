...
describe('Timesheet Submission Integration', () => {
  ...
  describe('Database Update Validation and Persistence', () => {
    it('should validate row count when marking entries as submitted', () => {
      ...
      const completeEntries = db.prepare('SELECT * FROM timesheet WHERE status IS NOT NULL').all() as Array<{
    ...
    it('should persist changes to disk with WAL checkpoint', () => {
      ...
      const db = openDb();
      const entry = db.prepare('SELECT * FROM timesheet WHERE id = ?').get(entryId) as {
        status: string | number;
        submitted_at: string;
      };
    ...
    it('should maintain referential integrity across submission lifecycle', () => {
      ...
      db.prepare("UPDATE timesheet SET status = ?, submitted_at = datetime('now') WHERE id = ?").run(
        'Complete',
        entryId
      );

      // Verify in archive
      const archiveEntry = db.prepare('SELECT * FROM timesheet WHERE id = ?').get(entryId) as {
        status: string;
        id: number;
        project: string;
      };
