...
export function assertValidTimesheetRow(row: TimesheetRow): void {
  ...
  expect(row.hours).toBeGreaterThanOrEqual(0.25);
  expect(row.hours).toBeLessThanOrEqual(24.0);
 ...
 */
export function assertInvalidTimesheetRow(
  row: TimesheetRow,
  ...
  if (row.hours === undefined || row.hours === null) {
    invalidFields.push("hours");
  } else if (!isValidHours(row.hours)) {
    // Check hours validity (15-minute increments, range)
    invalidFields.push("hours");
  }
  ...
  if (
    !row.taskDescription ||
    (typeof row.taskDescription === "string" &&
 ...
 */
export function assertTimeOutAfterTimeIn(
  timeIn: string,
  timeOut: string
): void {
...
export function assertValidDbEntry(entry: DbTimesheetEntry): void {
  ...
  if (entry.hours !== null && entry.hours !== undefined) {
    expect(entry.hours).toBeGreaterThanOrEqual(0.25);
    expect(entry.hours).toBeLessThanOrEqual(24.0);
 ...
 */
export function assertTimeConversionConsistency(
  timeStr: string,
  minutes: number
): void {
  const parts = timeStr.split(":").map(Number);
 ...
 */
export function assertDateConversionConsistency(
  dateStr: string,
  isoDate: string
): void {
  const parts = dateStr.split("/").map(Number);
...
export function assertValidIPCPayload(payload: Record<string, unknown>): void {
  ...
  expect(payload).toHaveProperty("hours");
  ...
  if (payload["chargeCode"] !== undefined) {
    expect(
      typeof payload["chargeCode"] === "string" ||
        payload["chargeCode"] === null
    ).toBe(true);
  }
}
...
export function assertUserFriendlyErrorMessage(errorMessage: string): void {
  ...
  const hasFriendlyPattern = friendlyPatterns.some((pattern) =>
 ...
 */
export function assertQuarterValidation(
  dateStr: string,
  ...
  if (month && day && year) {
    ...
    const isValid =
      entryYear === currentYear && entryQuarter === currentQuarter;
...
export function assertDatabaseConstraints(entry: DbTimesheetEntry): void {
  ...
  if (entry.hours !== null && entry.hours !== undefined) {
    ...
    if (entry.hours < 0.25 || entry.hours > 24.0) {
      throw new Error(
        `Invalid hours: ${entry.hours} (must be between 0.25 and 24.0)`
      );
    }

    // 15-minute increment constraint
    const remainder = (entry.hours * 4) % 1;
    if (Math.abs(remainder) > 0.0001 && Math.abs(remainder - 1) > 0.0001) {
      throw new Error(`Hours must be in 15-minute increments: ${entry.hours}`);
    }
  }
}
 ...
 */
export function assertPluginInterface(
  ...
  const methods =
    requiredMethods[interfaceName as keyof typeof requiredMethods];
