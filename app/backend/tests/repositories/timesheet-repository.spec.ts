...
} from "../../src/models/timesheet-repository";
import {
  setDbPath,
  openDb,
  ensureSchema,
...
describe("Timesheet Repository", () => {
  ...
  beforeEach(() => {
    ...
    testDbPath = path.join(
      os.tmpdir(),
      `sheetpilot-timesheet-repo-test-${Date.now()}.sqlite`
    );
  ...
  describe("Single Entry Operations", () => {
    ...
    it("should update entry status", () => {
      ...
      const db = openDb();
      const row = db
        .prepare("SELECT id FROM timesheet WHERE project = ?")
        .get("Test");
      ...
      const db2 = openDb();
      const updated = db2
        .prepare("SELECT status FROM timesheet WHERE id = ?")
        .get(entryId);
    ...
    it("should delete entry successfully", () => {
      ...
      const deleted = db
        .prepare("SELECT id FROM timesheet WHERE id = ?")
        .get(entryId);
  ...
  describe("Query Operations", () => {
    beforeEach(() => {
      ...
      const db = openDb();
      db.prepare("UPDATE timesheet SET status = ? WHERE project = ?").run(
        "Complete",
        "Project A"
      );
      db.close();
    ...
    it("should get only submitted entries for export", () => {
      ...
      const entry = submitted[0] as unknown as Record<string, unknown>;
  ...
  describe("Data Consistency", () => {
    it("should maintain referential integrity", () => {
      ...
      const db = openDb();
      const row = db
        .prepare("SELECT * FROM timesheet WHERE project = ?")
        .get("Integrity Test");
      db.close();
    ...
    it("should store hours correctly", () => {
      ...
      const db = openDb();
      const row = db
        .prepare("SELECT hours FROM timesheet WHERE project = ?")
        .get("Hours Test");
      db.close();
    ...
    it("should handle NULL values in optional fields", () => {
      ...
      const row = db
        .prepare(
          "SELECT tool, detail_charge_code FROM timesheet WHERE project = ?"
        )
        .get("Null Test");
      db.close();
