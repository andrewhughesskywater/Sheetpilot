...
import { ipcMain, IpcMainInvokeEvent, BrowserWindow, app as _app } from 'electron';
...
// Route handlers
import { registerAdminHandlers } from '../../src/routes/admin-handlers';
import { registerAuthHandlers } from '../../src/routes/auth-handlers';
import { registerCredentialsHandlers } from '../../src/routes/credentials-handlers';
import { registerDatabaseHandlers } from '../../src/routes/database-handlers';
import { registerAllIPCHandlers } from '../../src/routes/index';
import { registerTimesheetHandlers, setMainWindow } from '../../src/routes/timesheet-handlers';
import { registerLogsHandlers } from '../../src/routes/logs-handlers';
import { registerLoggerHandlers } from '../../src/routes/logger-handlers';
import { registerSettingsHandlers } from '../../src/routes/settings-handlers';

// Models/Repositories
import * as repositories from '../../src/models';
import * as repo from '../../src/models';
import {
  ensureSchema,
...
describe('auth-handlers', () => {
  ...
  describe('auth:login handler', () => {
    ...
    it('should login admin user successfully', async () => {
      ...
      vi.resetModules();
      const { registerAuthHandlers: registerAuth } = await import('../../src/routes/auth-handlers');
      
      // Clear and setup fresh handler capture
      handleCalls = [];
      vi.mocked(ipcMain.handle).mockImplementation((channel: string, handler: (event: IpcMainInvokeEvent, ...args: unknown[]) => unknown) => {
        handleCalls.push([channel, handler]);
        return undefined as never;
      });
