...
describe('Quarter Validation Unit Tests', () => {
  ...
  describe('Quarter Validation Logic', () => {
    it('debug: log quarter validation for candidate dates', () => {
      const quarter =
        QUARTER_DEFINITIONS.find((q) => new Date(q.endDate) >= new Date(q.startDate)) || QUARTER_DEFINITIONS[0];
  ...
  describe('Quarter Transition Handling', () => {
    it('should handle quarter transitions correctly (dynamic)', () => {
      ...
      for (let i = 0; i < QUARTER_DEFINITIONS.length - 1; i++) {
        ...
        console.log(
          '[quarter-debug] transition cur=',
          cur.id,
          'curIso=',
          curIso,
          'validate=',
          validateQuarterAvailability(curIso)
        );
        console.log(
          '[quarter-debug] transition next=',
          next.id,
          'nextIso=',
          nextIso,
          'validate=',
          validateQuarterAvailability(nextIso)
        );
        if (validateQuarterAvailability(curIso) !== null)
          throw new Error(
            `[quarter-debug] transition curIso=${curIso} returned ${validateQuarterAvailability(curIso)}`
          );
        if (validateQuarterAvailability(nextIso) !== null)
          throw new Error(
            `[quarter-debug] transition nextIso=${nextIso} returned ${validateQuarterAvailability(nextIso)}`
          );
        found = true;
        break;
      }
      ...
      if (!found) {
        ...
        expect(
          validateQuarterAvailability(
            `${safeStart.getFullYear()}-${String(safeStart.getMonth() + 1).padStart(2, '0')}-${String(safeStart.getDate()).padStart(2, '0')}`
          )
        ).toBeNull();
        expect(
          validateQuarterAvailability(
            `${safeEnd.getFullYear()}-${String(safeEnd.getMonth() + 1).padStart(2, '0')}-${String(safeEnd.getDate()).padStart(2, '0')}`
          )
        ).toBeNull();
      }
    });
  ...
  describe('Edge Cases', () => {
    it('should handle timezone differences', () => {
      ...
      const dates = ['2025-01-01T00:00:00Z', '2025-01-01T23:59:59Z', '2025-03-31T00:00:00Z', '2025-03-31T23:59:59Z'];
    ...
    it('should handle invalid date inputs gracefully', () => {
      const invalidInputs = ['not-a-date', '2025-13-01', '2025-02-30', '2025-04-31', '', null, undefined];
