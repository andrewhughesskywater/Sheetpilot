...
import { setDbPath, ensureSchema, getDb } from '@/repositories';
...
export function createTestDatabase(): string {
  const testDbPath = path.join(
    os.tmpdir(),
    `sheetpilot-test-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.sqlite`
  );
...
export function getDatabaseStats(): { count: number; projects: string[]; dateRange: { min: string; max: string } } {
  ...
  try {
    ...
    const projects = db.prepare('SELECT DISTINCT project FROM timesheet ORDER BY project').all() as {
      project: string;
    }[];
    const dateRange = db.prepare('SELECT MIN(date) as min, MAX(date) as max FROM timesheet').get() as {
      min: string;
      max: string;
    };
