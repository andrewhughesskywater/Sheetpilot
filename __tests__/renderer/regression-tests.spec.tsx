...
// ... rest of main.tsx content using import.meta.env.DEV`;
    ...
    if (path.includes('dev-performance-suppressor.ts')) {
  ...
  readdirSync: vi.fn((_dir?: string) => ['main.tsx', 'App.tsx', 'dev-performance-suppressor.ts']),
...
describe('Regression Tests for Fixed Files', () => {
  ...
  it('should not contain process.env in dev-performance-suppressor.ts', () => {
    const devSuppressorPath = mockPath.join(mockProcess.cwd(), 'src', 'renderer', 'dev-performance-suppressor.ts');
    const content = mockFs.readFileSync(devSuppressorPath, 'utf-8');
